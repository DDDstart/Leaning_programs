...
list.insert(y, x) — вставляет элемент x в список с порядковым номером y.
list.remove(x) — удаляет первый элемент в списке, который равен значению x.
list.pop(y) — удаляет элемент списка по порядковому номеру y.
list.index(g, x, y) — возвращает порядковый номер первого элемента со значением g в списке в диапазоне от x до y.
